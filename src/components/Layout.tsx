...
import { Menu, X, BookOpen, Loader2, Building2 } from "lucide-react";
...
import { createMarketingContact } from "../lib/zoho";
...
export function Layout() {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createMarketingContact({ email: newsletterEmail, source: "footer", consent: true });
  ...
  const servicesSection = navSections.find((section) => section.id === "services");
    ...
    <div className="font-sans antialiased text-brand-slate bg-brand-cream min-h-screen flex flex-col">
      ...
      >
        <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex flex-1 justify-center">
            <NavigationMenu viewport={true}>
              <NavigationMenuList>
                {whoWeHelpSection ? (
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className="bg-transparent text-white hover:text-white hover:bg-white/20 focus:bg-white/20 focus:text-white data-[active]:bg-white/20 data-[state=open]:bg-white/20 data-[state=open]:text-white h-11">
                      {whoWeHelpSection.label}
                    </NavigationMenuTrigger>
                    <NavigationMenuContent>
                      <ul className="grid w-[400px] gap-2 p-2 md:w-[700px] md:grid-cols-2 lg:w-[1000px] lg:grid-cols-4">
                        {whoWeHelpSection.featured ? (
                          <li className="row-span-4 lg:row-span-2">
                            <NavigationMenuLink asChild>
                              <NavLink
                                to={whoWeHelpSection.featured.to ?? "/"}
                                className="group flex h-full w-full select-none flex-col justify-end rounded-lg bg-gradient-to-b from-brand-copper/30 via-brand-copper/50 to-brand-oxford p-8 no-underline outline-none hover:from-brand-copper/40 hover:via-brand-copper/60 hover:to-brand-oxford transition-all duration-300 cursor-pointer border border-brand-copper/20 shadow-md hover:shadow-lg min-h-[200px]"
                              >
                        ...
                        <div className="col-span-1 md:col-span-1 lg:col-span-3 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-2">
                ...
                {servicesSection ? (
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid w-[400px] gap-2 p-2 md:w-[700px] md:grid-cols-2 lg:w-[800px] lg:grid-cols-3">
                ...
                {platformSection ? (
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid w-[400px] gap-2 p-2 md:w-[600px] md:grid-cols-2">
                ...
                {resourcesSection ? (
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid w-[400px] gap-2 p-2 md:w-[600px] md:grid-cols-2 bg-white rounded-md">
                        {resourcesSection.featured ? (
                          <li className="col-span-1 md:col-span-2">
                            <NavigationMenuLink asChild>
                              <NavLink
                                to={resourcesSection.featured.to ?? "/"}
                                className="flex w-full select-none flex-col rounded-lg bg-gradient-to-br from-brand-copper/10 to-brand-copper/5 p-4 no-underline outline-none hover:from-brand-copper/20 hover:to-brand-copper/10 transition-all duration-300 cursor-pointer border border-brand-copper/20 shadow-sm hover:shadow-md"
                              >
                                <div className="flex items-center gap-2 text-brand-oxford">
                                  <BookOpen size={18} className="text-brand-copper" />
                                  <div className="text-base font-bold">
                                    {resourcesSection.featured.title}
                                  </div>
                                </div>
                ...
                {companySection ? (
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid gap-2 p-2 md:w-[500px] lg:w-[800px] lg:grid-cols-[.75fr_1fr]">
                        {companySection.featured ? (
                          <li className="row-span-3">
                            <NavigationMenuLink asChild>
                              <NavLink
                                to={companySection.featured.to ?? "/"}
                                className="flex h-full w-full select-none flex-col justify-between rounded-lg bg-gradient-to-br from-brand-copper via-brand-copper to-brand-copper-dark p-4 no-underline outline-none hover:shadow-lg transition-all duration-300 cursor-pointer border border-brand-copper shadow-md"
                              >
          ...
          <div className="hidden md:flex items-center gap-3 shrink-0 ml-4">
            <Button
              variant="ghost"
              className="text-white hover:text-white hover:bg-white/10 font-medium h-11 hidden lg:inline-flex"
              asChild
            >
              <Link to={ctaLinks.support.to}>{ctaLinks.support.label}</Link>
            </Button>
            <Button
              className="bg-brand-copper hover:bg-brand-copper-dark text-white font-medium rounded-md transition-all shadow-md hover:shadow-lg h-11"
              asChild
            >
      ...
      <footer className="bg-brand-oxford text-gray-400 py-12 border-t border-brand-copper">
        <div className="container mx-auto px-6">
          ...
          <div className="flex flex-col gap-2 items-center md:items-start text-center md:text-left border-t border-gray-700 pt-8 mt-4">
          ...
          <div className="flex flex-wrap justify-center md:justify-end items-center gap-x-4 md:gap-x-6 lg:gap-x-8 gap-y-3 mt-6">
      ...
      <li>
        <NavigationMenuLink asChild>
          <div
            ...
            aria-disabled="true"
          >
            <div className="text-base font-bold leading-none">{item.title}</div>
            {item.description ? (
      ...
      <li>
        <NavigationMenuLink asChild>
          <a
            ...
            rel={item.external ? "noopener noreferrer" : undefined}
            className={cn(
              "block select-none space-y-1 rounded-md p-2 leading-none no-underline outline-none transition-colors hover:bg-gray-100 cursor-pointer",
              className
            )}
          >
            <div className="text-base font-bold leading-none text-brand-oxford">{item.title}</div>
            {item.description ? (
    ...
    <li>
      <NavigationMenuLink asChild>
        <NavLink
          ...
          className={({ isActive }) =>
            cn(
              "block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-gray-100 cursor-pointer",
