...
export function Services({ onViewChange }: ServicesProps) {
  useEffect(() => {
    document.title = "Humaneers | Our Services";
    window.scrollTo(0, 0);
  }, []);
    ...
    <div className="bg-white">
      ...
      <section className="bg-[#1B263B] text-white py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="inline-flex items-center justify-center p-3 bg-[#B87333]/20 rounded-full mb-6">
              <Layers className="w-6 h-6 text-[#B87333] mr-2" />
              <span className="text-[#B87333] font-bold uppercase tracking-widest text-sm">Full Spectrum Support</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6">Comprehensive Services<br/>for Modern Growth.</h1>
          <p className="text-xl md:text-2xl text-gray-300 font-light leading-relaxed max-w-3xl mx-auto mb-8">
            From the server room to the board room, we provide the technology and strategy you need to scale.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service) => (
              <div key={service.id} className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col group hover:shadow-xl transition-shadow">
                <div className="h-64 overflow-hidden relative">
                    ...
                    />
                    <div className={`absolute inset-0 ${service.color} opacity-80 mix-blend-multiply`} />
                    <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black/50 to-transparent">
                      <div className="flex items-center gap-4">
                        <div className="bg-white/20 p-2 rounded-xl backdrop-blur-sm">{service.icon}</div>
                      ...
                      <ul className="space-y-3 mb-8">
                         {service.features.map((feature, i) => (
                            <li key={i} className="flex items-center gap-3 text-[#1B263B] font-medium">
                               <CheckCircle2 className="w-5 h-5 text-[#B87333] shrink-0" /> {feature}
                            </li>
                         ))}
                      </ul>
                   </div>
                   <Button 
                      onClick={() => onViewChange(service.link)}
                      className="w-full bg-gray-100 text-[#1B263B] hover:bg-gray-200 justify-between group-hover:bg-[#B87333] group-hover:text-white transition-colors"
                   >
      ...
      <section className="py-24 bg-white">
         <div className="container mx-auto px-6 text-center">
            <h2 className="text-3xl font-bold text-[#1B263B] mb-12">Why Clients Choose Us</h2>
            <div className="grid md:grid-cols-3 gap-8">
               <div className="p-6">
                  <div className="text-4xl font-bold text-[#B87333] mb-4">100%</div>
                  <h3 className="text-xl font-bold text-[#1B263B] mb-2">US-Based</h3>
                  <p className="text-[#4E596F]">No offshore call centers. Every engineer and strategist is local.</p>
               </div>
               <div className="p-6">
                  <div className="text-4xl font-bold text-[#B87333] mb-4 flex items-center justify-center gap-2">
                     <DefinitionTooltip term="SOC 2" definition="System and Organization Controls 2: An auditing procedure that ensures service providers securely manage your data to protect the interests of your organization and the privacy of its clients." />
                  </div>
                  <h3 className="text-xl font-bold text-[#1B263B] mb-2">Compliant</h3>
                  <p className="text-[#4E596F]">Enterprise-grade security controls applied to every client, big or small.</p>
               </div>
               <div className="p-6">
                  <div className="text-4xl font-bold text-[#B87333] mb-4">No</div>
                  <h3 className="text-xl font-bold text-[#1B263B] mb-2">Vendor Lock-in</h3>
                  <p className="text-[#4E596F]">We build on standard stacks (Microsoft, AWS) so you own your infrastructure.</p>
               </div>
            </div>
         </div>
      </section>
