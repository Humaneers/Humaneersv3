...
/**
 * Submit form data to the backend proxy (Native API Submission)
 */
export async function submitFormToProxy(
  type: "sales" | "support" | "newsletter",
  data: SalesFormData | SupportFormData | NewsletterFormData
): Promise<any> {
  const config = getCalConfig();
  let formId: string | undefined;

  // Prepare data payload based on type
  // We use the same parameter mapping as the redirect method to ensure consistency
  // with how Cal.com expects field names.
  let payload: Record<string, any> = {};

  if (type === "sales") {
    formId = config.salesFormId;
    const salesData = data as SalesFormData;
    payload = {
      firstName: salesData.firstName,
      lastName: salesData.lastName,
      ...
      role: salesData.role,
      employees: salesData.employees,
      website: salesData.website,
      phone: salesData.phone,
      budget: salesData.budget,
    ...
    if (
      ...
      payload.interests = salesData.interests.join(", ");
    }
  } else if (type === "support") {
    formId = config.supportFormId;
    const supportData = data as SupportFormData;
    payload = {
      name: supportData.name,
      email: supportData.email,
    ...
    const newsletterData = data as NewsletterFormData;
    payload = {
  ...
  if (!formId) {
    throw new Error(`${type.charAt(0).toUpperCase() + type.slice(1)} form ID is not configured`);
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(error.error || `Failed to submit ${type} form`);
  }
